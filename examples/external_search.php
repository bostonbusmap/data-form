...
function make_external_search_form($state) {
	...
	$settings = DataTableSettingsBuilder::create()->total_rows($num_rows)->default_limit(10)->build();
	...
	$columns[] = DataTableColumnBuilder::create()->display_header_name("Scientific name")->column_key("organism_scientific")->build();
	...
	$widgets[] = DataTableSearchWidgetBuilder::create()->column_key("organism_name")->search_type(DataTableSearchState::rlike)->
		label("Search organism name: ")->build();



	$table = DataTableBuilder::create()->columns($columns)->rows(new DatabaseIterator($query, null, "organism_name"))->
